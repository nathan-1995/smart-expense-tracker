...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // Load clients and invoice in parallel
        const [clientsResponse, invoice] = await Promise.all([
        ...
        if (isSubscribed) {
          ...
          setTaxRate(invoice.tax_rate ? parseFloat(invoice.tax_rate.toString()) : 0);
          setDiscountAmount(invoice.discount_amount ? parseFloat(invoice.discount_amount.toString()) : 0);
  ...
  const calculateTotal = () => {
    return calculateSubtotal() + calculateTax() - Number(discountAmount);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(apiError.response?.data?.detail || "Failed to update invoice");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold mt-2">Edit Invoice</h1>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Update items on your invoice</CardDescription>
                </div>
